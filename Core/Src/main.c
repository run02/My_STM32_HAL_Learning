...
#define REC_LENGTH 5
unsigned char UART1_Rx_Buf[MAX_REC_LENGTH] = {0}; //USART1瀛樺偍鎺ユ敹鏁版嵁
unsigned char UART1_Rx_flg = 0;                   //USART1鎺ユ敹瀹屾垚鏍囧織
unsigned int  UART1_Rx_cnt = 0;                   //USART1鎺ュ彈鏁版嵁璁℃暟锟??
unsigned char UART1_temp[REC_LENGTH] = {0};       //USART1鎺ユ敹鏁版嵁缂撳瓨
...
//strcpy(Tube_String8,"123456\0");
  ...
  while (1)
  {
      ...
      play_num_it(1,3,TIM2->CCR2*100/TIM2->ARR);
      play_num_it(4,7,8000000/((TIM2->PSC+1)*(TIM2->ARR+1)));
      HAL_Delay(1);
      play_string_it(0,"C");
    /* USER CODE END WHILE */
